...
### SERVER
setwd("/run/user/1000/gvfs/sftp:host=storage-brno1-cerit.metacentrum.cz/nfs4/home/agata/infernal_insects/results/0710_bombus3_chalcidoidea3/")

manual_fill <- ("/run/user/1000/gvfs/sftp:host=storage-brno1-cerit.metacentrum.cz/nfs4/home/agata/infernal_insects/manual_fill/")
results <- ("/run/user/1000/gvfs/sftp:host=storage-brno1-cerit.metacentrum.cz/nfs4/home/agata/infernal_insects/results/0710_bombus3_chalcidoidea3/")
labels_dir <- ("/run/user/1000/gvfs/sftp:host=storage-brno1-cerit.metacentrum.cz/nfs4/home/agata/infernal_insects/")
...
dirs = list.dirs(path = ".", full.names = TRUE, recursive = FALSE)     ### make a list of avaible directories
...
for (folder in dirs) {
  ...
  for (file in files) {
    ...
    # file = "result_U4_vs_GCA_900474335.1_Onit_genomic.csv"
...
}   ### END OF THE FOLDERS LOOPit seems 
...
############################  SAVE NAMES OF FILES THAT NEED TO BE FILLED MANUALLY ###########################
labels <- read_csv(paste0(labels_dir, "labels.csv"))
...
write_csv(wynik, paste0(results, "/output_table.csv"))

filtered <- wynik %>% 
  filter(label == "HIT" & suma > 0)

write_csv(filtered, paste0(results, "/output_table_filtered.csv"))
